...
// Admin Portal types and helpers
...
/**
 * Resolve an admin record for a specific authenticated user.
 *
 * Returns null for normal users. A database/RLS error is surfaced because
 * silently treating a broken admin policy as "not an admin" would hide setup
 * problems and lock the owner out of the portal.
 */
...
/**
 * Check the currently authenticated Supabase session for admin access.
 */
export async function getCurrentAdmin(): Promise<AdminUser | null> {
  ...
  if (userError) {
    throw new Error(`Could not read the current session: ${userError.message}`);
  }
...
/**
 * Count one table without downloading its rows.
 */
async function countAdminTable(
  table: string,
...
/**
 * Load the top-level statistics used by the Admin Dashboard.
 *
 * This should only be called after fetchAdminUser/getCurrentAdmin confirms
 * access. Database RLS remains the final security boundary.
 */
export async function fetchAdminDashboardStats(): Promise<AdminDashboardStats> {
  ...
  const [
    ...
    countAdminTable('worker_profiles'),
    countAdminTable('contractor_profiles'),
    countAdminTable('jobs'),
    countAdminTable('matches'),
    countAdminTable('messages'),
    countAdminTable('interviews'),
    countAdminTable('reviews'),
    countAdminTable('reviews', query => query.eq('reported', true)),
    countAdminTable(
      'worker_profiles',
  ...
  return {
    totalUsers: workers + contractors,
    workers,
    contractors,
    liveJobs,
    matches,
...
export async function fetchAdminManagedUsers(): Promise<AdminManagedUser[]> {
  ...
  // Select the complete rows rather than naming optional columns.
  // This supports both the original HireUp schema and newer column aliases.
  const [
  ...
  const resolveAccountStatus = (row: any): AdminAccountStatus => {
    const rawStatus = row.account_status || row.status || 'active';

    if (rawStatus === 'suspended') return 'suspended';
    if (rawStatus === 'banned' || rawStatus === 'disabled') return 'banned';
...
export async function updateAdminVerificationStatus(
  ...
  // The live HireUp schema stores verification in verified_status.
  // Keep the update limited to columns that actually exist.
  const { error } = await supabase
    .from(table)
    .update({
